...
public class AuditAspect {
    ...
    @AfterReturning(value = "@annotation(auditable)", returning = "result") // law el method done normally
    public void auditSuccess(JoinPoint jp, Auditable auditable, Object result) {
        ...
        log.info(
                "AUDIT | timestamp={} | user={} | role={} | action={} | resource={} | method={} | params={} | status=SUCCESS",
                LocalDateTime.now(), userId, userRole, auditable.action(), auditable.resourceType(), methodName, args);
    }

    @AfterThrowing(value = "@annotation(auditable)", throwing = "ex") // law el method failed aw thrown any exception
    public void auditFailure(JoinPoint jp, Auditable auditable, Exception ex) {
        ...
        log.warn(
                "AUDIT | timestamp={} | user={} | role={} | action={} | resource={} | method={} | params={} | status=FAILURE | error={}",
                LocalDateTime.now(), userId, userRole, auditable.action(), auditable.resourceType(), methodName, args,
                ex.getMessage());
    }

    private String extractHeader(String headerName) { // bnggeb el header name from original request
        try {
